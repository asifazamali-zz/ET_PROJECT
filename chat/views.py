...
def assignment(l,correct):
	...
	if correct==0:
		...
		#print correct_len
		print 'allocated_to_b,allocated_to_c,allocated_to_d',allocated_to_b,allocated_to_c,allocated_to_d
		index_list=[]
		if allocated_to_b>0:
		...
		print "allocated_index", allocated_index
...
def chat_room(request):
	...
	if posted:
		...
		correct =question[0].option_correct
        ...
        #else:
		########### For mapped users with options########################
			mapped_users={}
			print 'list a-d',list_a,list_b,list_c,list_d
			for item in output:
				if request.user.username in item:
					for element in item:
						if element in list_a:
							mapped_users.update({element:'a'})

						if element in list_b:
							mapped_users.update({element:'b'})

						if element in list_c:
							mapped_users.update({element:'c'})

						if element in list_d:
							mapped_users.update({element:'d'})
